...
We are going to remove the first 26 popular words from two party, as the these words are overlap about 88.5%.   

Now, the President in different party looks like have different emphasis. The highest frequent words in Democratic is "Power" and in Republican is "Law".
...
```{r, echo=FALSE, fig.height=2, fig.width=3}
word_num <- NULL 
list = c("america", "power", "law", "war", "women","tax", "freedom")
...
The most obvious trend is "america" became much more prevalent as time went on. It is possible that the purpose of inaugural addresses has changed over time. Maybe the President are more likely to talk foreign policy and the world as a whole, so the use of "america" to specify what th President is talking about. 

The trend of law were increased and then decreased, it may explained by the process of law reform. Nowadays, the law in U.S. are more complete compare with law in 1900s. 

It is interest that the frequency of the word "war" in these speeches are decreased, even though in recent years the conflicts in the Middle East is actually increased. 

The first "women" been mentioned after 1900s is actually the time women in U.S. got their rights. Women get right at August 18, 1920. 
...
By comparing the two party, it also have obvious trend in it. The Republican mentioned "war" and "law" more than Democratic. 
...
ggplot() +
  geom_point(aes(x = dat$Year, y = dat$freedom, color = "freedom.dem")) +  
  geom_line(aes(x = dat$Year, y = dat$freedom, color = "freedom.dem")) +
  geom_point(aes(x = dat2$Year, y = dat2$freedom, color = "freedom.rep")) +
  geom_line(aes(x = dat2$Year, y = dat2$freedom, color = "freedom.rep"))
...
The curve is obvious, Presidents in age around 60 do not gave too much words in addresses. Age after 60 the words are increasing, which may explained by as people getting old, they talk more. 
